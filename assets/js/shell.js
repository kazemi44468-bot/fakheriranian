...
(function(){
  ...
  var links = [
    ['نخست','index.html','home'],
    ['فروش','pages/sales.html','sales'],
    ['پرو آنلاین','pages/virtual-fitting.html','virtual'],
    ['سفارش','pages/order.html','order'],
    ['درباره ما','pages/about.html','about'],
    ['تماس با ما','pages/contact.html','contact']
  ];
  var active = file === 'index.html' ? 'home' : file.replace('.html','');
  function href(path){ return root + path; }
  function navHtml(){
    return links.map(function(x){
      return '<a'+(x[2]===active?' class="active"':'')+' href="'+href(x[1])+'">'+x[0]+'</a>';
    }).join('');
  }
  function shell(){
    ...
    header.innerHTML='<div class="header-inner"><nav class="nav right">'+navHtml().split('</a>')[0]+'</a>'+navHtml().split('</a>').slice(1,3).join('</a>')+'</a></nav><a class="brand" href="'+href('index.html')+'" aria-label="گروه پوشاک فاخر ایرانیان"><img src="'+href('assets/images/logo.png')+'" alt="گروه پوشاک فاخر ایرانیان"></a><nav class="nav left">'+navHtml().split('</a>').slice(3).join('</a>')+'</nav><button class="mobile-trigger" type="button" aria-label="منوی موبایل" aria-expanded="false"></button></div>';
    document.body.insertBefore(header, document.body.firstElementChild);
    var drawer=document.createElement('aside');
    drawer.className='mobile-drawer'; drawer.id='mobileDrawer'; drawer.setAttribute('aria-label','منوی موبایل');
    drawer.innerHTML='<button class="drawer-close" type="button" aria-label="بستن">×</button><img class="mobile-drawer-logo" src="'+href('assets/images/logo.png')+'" alt="فاخر"><nav>'+navHtml()+'</nav>';
